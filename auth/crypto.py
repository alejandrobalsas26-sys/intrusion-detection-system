    ...
    def __init__(self):
        ...
        if not key or key == "your_fernet_key_here":
            raise ValueError(
                ...
                "Generate one using: python -c 'from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())'"  # noqa: E501
            )

        try:
            self.cipher = Fernet(key.encode())
        except Exception as e:
            raise ValueError(f"Invalid MFA_ENCRYPTION_KEY: {e}")  # noqa: B904
